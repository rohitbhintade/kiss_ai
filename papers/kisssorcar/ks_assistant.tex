...
\paragraph{Limitations.}  Our evaluation uses a single frontier model (Claude Opus~4.6) and a single benchmark (Terminal Bench~2.0).  Evaluating across a broader set of models (including open-weight models) and benchmarks (including SWE-bench Pro and LiveCodeBench) is important future work.  The continuation protocol's structured summaries may lose fidelity for very long tasks spanning many sub-sessions; while we have not observed this in practice, a systematic study of summary degradation over many continuations remains open.  The quality-over-speed posture---giving the agent time to run linters, type checkers, and tests---means that task completion times are longer than those of agents that skip verification; this trade-off may not suit latency-sensitive workflows.  Finally, the no-mocks testing discipline, while producing high-confidence test suites, can be impractical for systems with expensive external dependencies (e.g., paid third-party APIs).
