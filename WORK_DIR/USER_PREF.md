...
- `.gitattributes` at repo root sets `WORK_DIR/USER_PREF.md merge=union` so concurrent append-only edits from different worktrees auto-concatenate without manual conflict resolution.
- `GitWorktreeOps.cleanup_orphans()` also scans `.kiss-worktrees/` for subdirectories not registered as git worktrees and removes them (BUG: leftover directories from crashed sessions or manual `.git` unlinks were never cleaned). Pending-merge worktrees (registered + `kiss-original` config) are still kept per BUG-58.
- Non-worktree "Auto commit" flow shows VS Code notifications just like the worktree "Auto-commit and merge" flow. `server.py::_handle_autocommit_action` emits 3 `autocommit_progress` events ("Staging changes…", "Generating commit message…", "Committing…") then `autocommit_done`. `SorcarSidebarView.ts` wraps the commit in `vscode.window.withProgress({Notification, title:'Auto-committing…'})` keyed by `tabId` via `_autocommitProgresses` / `_autocommitActionResolves` maps (same pattern as `_worktreeProgresses` / `_worktreeActionResolves`), and on `autocommit_done` shows `showInformationMessage` on success or `showErrorMessage` on failure. `media/main.js::handleAutocommitResult` calls `focusInputWithRetry()` so the input textarea regains focus once the commit finishes.
- `src/kiss/channels/cron_manager_daemon.py` is a Unix-socket daemon (`CronDaemon` + `CronClient`) that delegates all scheduling/execution/persistence to the system `crontab` command. The daemon owns no job state; each request is translated into one or two `crontab -l` / `crontab -` invocations under a mutex. KISS-owned crontab entries are framed by `# KISS-JOB <id> <created_at>` comments so user entries are preserved. `CronDaemon.__init__` accepts `crontab_cmd` so tests can inject a fake binary (a shebang Python script that reads/writes a per-test tab file) — no mocks or test doubles. `stop_daemon()` uses PID-file disappearance (daemon removes it during `_cleanup` before exit) as a secondary shutdown signal, because `os.kill(zombie_pid, 0)` returns success for unreaped zombies.
